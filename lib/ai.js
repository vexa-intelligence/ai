...
export async function completeWithAIStream(prompt, messages, model, onChunk) {
    ...
    if (route === "aifree") return await aiFreeCompleteStream(prompt, msgs, onChunk);
    if (route === "toolbaz") return await toolbazCompleteStream(prompt, model, onChunk);
    throw new Error(`No provider found for model: ${model || "default"}`);
}
